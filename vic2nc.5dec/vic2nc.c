...
int main(int argc, char *argv[])
{
  ...
  for (fidx=0; fidx<NFILES; fidx++) {
    ...
    for(t=0; t<NRECS; t++) {
      ...
      if (!(LEAPYR(curr_rec_time->tm_year+1900) && curr_rec_time->tm_mon == 1 && curr_rec_time->tm_mday == 29)
          || !NO_LEAP_OUT) {
...
//  time_t *tp;
  ...
  char endian_str[10];
  long long dsec;
  const char year_unit_strs[][10] = {"year","years","yr","yrs","y","ys","yearly","",""};
  const char month_unit_strs[][10] = {"month","months","mon","mons","mo","mos","m","ms","monthly"};
  const char day_unit_strs[][10] = {"day","days","dy","dys","d","ds","daily","",""};
  const char hour_unit_strs[][10] = {"hour","hours","hr","hrs","h","hs","hourly","",""};
  const char min_unit_strs[][10] = {"minute","minutes","min","mins","mi","mis","mn","mns","minutely"};
  const char sec_unit_strs[][10] = {"second","seconds","sec","secs","s","ss","secondly","",""};
  int time_units_match;

  // Initialize attributes
  for (i=0; i<NGLOBALS; i++) {
    strcpy(global_atts[i],"");
  }
  strcpy(data_start_date_str,"");
  strcpy(data_start_time_str,"");
  strcpy(data_end_date_str,"");
  strcpy(data_end_time_str,"");
  *dtp = 0;
  strcpy(time_units,"");
  NLEVELS = 0;
  for (i=0; i<MAXLEV; i++) {
  ...
  strcpy(lev_units,"");
  strcpy(lev_long_name,"");
  strcpy(lev_positive,"");
  strcpy(FORMAT,"");
  strcpy(endian_str,"");
  for (i=0; i<MAXVAR; i++) {
    strcpy(var_atts[i].read,"");
    strcpy(var_atts[i].name,"");
    strcpy(var_atts[i].units,"");
    strcpy(var_atts[i].z_dep,"");
    var_atts[i].mult = 0;
    strcpy(var_atts[i].sign,"");
    strcpy(var_atts[i].long_name,"");
  }
  ...
  while (!feof(mf)) {
    if (cmdstr[0]!='#' && cmdstr[0]!='\n' && cmdstr[0]!='\0') {
      ...
      if (strcasecmp("INSTITUTION",optstr)==0) {
        ...
        sscanf(cmdstr,"%*s %d", &GRID_DECIMAL);
      }
        ...
        if(!strcasecmp(var_atts[varcount].z_dep,"TRUE") && NLEVELS == 0) {
          fprintf(stderr, "Error: variable %s is listed as having a dependence on z dimension, but NLEVELS is set to %d in the metadata file %s\n", var_atts[varcount].name, NLEVELS, metafile);
          exit(1);
        }
  ...
  time_units_match = 0;
  for (i=0; i<9; i++) {
    if (!strcasecmp(time_units,year_unit_strs[i])) {
      sprintf(time_units,"year");
      time_units_match = 1;
    }
  }
  for (i=0; i<9; i++) {
    if (!strcasecmp(time_units,month_unit_strs[i])) {
      sprintf(time_units,"month");
      time_units_match = 1;
    }
  }
  for (i=0; i<9; i++) {
    if (!strcasecmp(time_units,day_unit_strs[i])) {
      sprintf(time_units,"day");
      time_units_match = 1;
    }
  }
  for (i=0; i<9; i++) {
    if (!strcasecmp(time_units,hour_unit_strs[i])) {
      sprintf(time_units,"hour");
      time_units_match = 1;
    }
  }
  for (i=0; i<9; i++) {
    if (!strcasecmp(time_units,min_unit_strs[i])) {
      sprintf(time_units,"min");
      time_units_match = 1;
    }
  }
  for (i=0; i<9; i++) {
    if (!strcasecmp(time_units,sec_unit_strs[i])) {
...
struct GridInfo *Calc_Grid(char *inpath, char *prefix)
{
  ...
  char **latlonstr;
  double lat, lon, minlat, maxlat, minlon, maxlon, delta_lat, delta_lon, temp;
  double *lats, *lons, *latlon;
  ...
  // Allocate arrays
  lats = (double *)calloc(NCELLS,sizeof(double));
  lons = (double *)calloc(NCELLS,sizeof(double));
  latlon = (double *)calloc(NCELLS,sizeof(double));
  ...
  while((filep = readdir(dp)) != NULL) {
    if (!strncmp(filep->d_name,prefix,strlen(prefix))) {
      ...
      lat = ( atof(tok) + 100 ) * multiplier;
      tok = strtok(NULL, "_");
      lon = ( atof(tok) + 360 ) * multiplier;
  ...
  for (k=0; k<NCELLS; k++) {
      ...
      lats[k] = ( atof(tok) / multiplier ) - 100;
      tok = strtok(NULL, ".");
      lons[k] = ( atof(tok) / multiplier ) - 360;
  }
  ...
  if (NCELLS > 1) {
    ...
    for (k=0; k<NCELLS; k++) {
      if (first_time) {
        ...
        if (fabs(lats[k] - temp) >= 0.00001 && delta_lat > fabs(lats[k] - temp)) {
    ...
    for (k=0; k<NCELLS; k++) {
      if (first_time) {
        ...
        if (fabs(lons[k] - temp) >= 0.00001 && delta_lon > fabs(lons[k] - temp)) {
...
void Check_VIC_Files(char *inpath, char *prefix, struct GridInfo *grid, FILE **infilehandle)
{
  ...
  for (k=0; k<NCELLS; k++) {
    ...
    if (GRID_DECIMAL == 0)
      sprintf(filename, "%s/%s_%.0f_%.0f",inpath, prefix, grid[k].lat, grid[k].lon);
    else if (GRID_DECIMAL == 1)
      sprintf(filename, "%s/%s_%.1f_%.1f",inpath, prefix, grid[k].lat, grid[k].lon);
    else if (GRID_DECIMAL == 2)
      sprintf(filename, "%s/%s_%.2f_%.2f",inpath, prefix, grid[k].lat, grid[k].lon);
    else if (GRID_DECIMAL == 3)
      sprintf(filename, "%s/%s_%.3f_%.3f",inpath, prefix, grid[k].lat, grid[k].lon);
    else if (GRID_DECIMAL == 4)
      sprintf(filename, "%s/%s_%.4f_%.4f",inpath, prefix, grid[k].lat, grid[k].lon);
    else if (GRID_DECIMAL == 5)
      sprintf(filename, "%s/%s_%.5f_%.5f",inpath, prefix, grid[k].lat, grid[k].lon);
    else
      sprintf(filename, "%s/%s_%.4f_%.4f",inpath, prefix, grid[k].lat, grid[k].lon);
...
struct tm *first_rec_time, int dt, char *time_units, int NFILES, int fidx, struct tm *file_end_time, struct tm *last_rec_time)
{
  ...
  if (!strcmp(timespan,"total") || fidx == NFILES-1) {
    ...
    if (!strcmp(timespan,"y")) {
      ndays = 364;
      if ( (LEAPYR(file_start_time->tm_year+1900) && (file_start_time->tm_mon == 0 || (file_start_time->tm_mon == 1 && file_start_time->tm_mday <= 28))) || (LEAPYR(file_start_time->tm_year+1900+1) && file_start_time->tm_mon == 2) ) {
        ndays++;
      }
    }
    else if (!strcmp(timespan,"m")) {
      ndays = dmonth[file_start_time->tm_mon]-1;
      if (LEAPYR(file_start_time->tm_year+1900) && file_start_time->tm_mon == 1) {
        ndays++;
      }
    }
...
long long dt2sec(struct tm rec_time, int dt, char *time_units)
{
  ...
  if (!strcmp(time_units,"year")) {
    ...
    for (i=0; i<dt; i++) {
      rec_time.tm_year++;
      ndays += 365;
      if ( (LEAPYR(rec_time.tm_year+1900-1) && (rec_time.tm_mon == 0 || (rec_time.tm_mon == 1 && rec_time.tm_mday <= 28)))
          || (LEAPYR(rec_time.tm_year+1900) && rec_time.tm_mon > 1) ) {
        ndays++;
      }
    }
    dsec = ndays*86400LL;
    ...
    for (i=0; i<dt; i++) {
      ...
      if ( LEAPYR(rec_time.tm_year+1900) && (rec_time.tm_mon-1 == 1 && rec_time.tm_mday <= 28) ) {
        ndays++;
      }
...
//HF, 02/13. Changed all the short integer variables to float variables
//becasue I'm storing the binary files as floats.
//I commented out some lines that dealt with the endian concept.
//*************************************************************************************************
  ...
  float **var3d, float ***var4d)
{
  ...
  char *tempstr;
  int my_endian;
  float stmp;//HF 02/13 signed short stmp;
  float ustmp;//HF 02/13 unsigned short ustmp;
  ...
  for (k=0; k<NCELLS; k++) {
    ...
    if (!strcasecmp(FORMAT,"BINARY")) {
      ...
      for (v=0; v<(NUM3d+NUM4d); v++) {
		...
		&& fread(&stmp,sizeof(float),1,infilehandle[k])//short int
              && !ferror(infilehandle[k]) )
            {
              if (my_endian != ENDIAN) {
                //stmp = ((stmp & 0xFF) << 8) | ((stmp >> 8) & 0xFF);HF 02/13
              }
            ...
            if ( !feof(infilehandle[k])
              && fread(&ustmp,sizeof(float),1,infilehandle[k])//unsigned short int
              && !ferror(infilehandle[k]) )
            {
              if (my_endian != ENDIAN) {
                //ustmp = ((ustmp & 0xFF) << 8) | ((ustmp >> 8) & 0xFF);HF 02/13
              }
...
//  fprintf(stderr,"Done defining dimensions\n");
  ...
  if (GRIDFILE) {
    ...
    tempint = NODATA_INT;
    status = nc_put_att_int(ncid, rowvarid, "_FillValue", NC_INT, 1, &tempint);  Handle_Error(status);
    ...
    tempint = NODATA_INT;
    status = nc_put_att_int(ncid, colvarid, "_FillValue", NC_INT, 1, &tempint);  Handle_Error(status);
  }
  ...
  status = nc_put_att_float(ncid, latvarid, "_FillValue", NC_FLOAT, 1, &tempfloat);  Handle_Error(status);
  ...
  tempfloat = NODATA_FLOAT;
  status = nc_put_att_float(ncid, lonvarid, "_FillValue", NC_FLOAT, 1, &tempfloat);  Handle_Error(status);
...
//  fprintf(stderr,"Done defining coordinate variables\n");
  ...
  tempint = NODATA_INT;
  status = nc_put_att_int(ncid, cellidvarid, "_FillValue", NC_INT, 1, &tempint);  Handle_Error(status);
  ...
  for(v=0; v<(NUM3d+NUM4d); v++) {
    if(!strcasecmp(var_atts[v].read,"TRUE")) {
      if(!strcasecmp(var_atts[v].z_dep,"FALSE")) {
        ...
        tempfloat = NODATA_FLOAT;
        status = nc_put_att_float(ncid, var_atts[v].id, "_FillValue", NC_FLOAT, 1, &tempfloat);  Handle_Error(status);
      }
      else {
        if (COMPRESS) {
          ndim=3;
...
long long diff_date(struct tm date1, struct tm date2) {
  ...
  // Compute number of days since 1900-00-00 for both dates
  ndays = julian(date1.tm_year, date1.tm_mon, date1.tm_mday);
  for (i=1; i<date1.tm_year; i++) {
    ndays += 365;
    if (LEAPYR(i+1900)) {
      ndays++;
    }
  ...
  ndays1 = ndays;
  ndays = julian(date2.tm_year, date2.tm_mon, date2.tm_mday);
  for (i=1; i<date2.tm_year; i++) {
    ndays += 365;
    if (LEAPYR(i+1900)) {
      ndays++;
    }
...
//*************************************************************************************************
//  LEAPYR: returns 1 if year is a leap year, 0 if not
//*************************************************************************************************
int LEAPYR(int year) {
...
void add_sec_date(struct tm start_date, long long sec_to_add, struct tm *new_date) {
  ...
  while (ndays) {
    ...
    days_in_year = 365;
    if (LEAPYR(new_date->tm_year+1900)) {
      // Leap year
      days_in_year++;
    }
    ...
    if (ndays > 0 && ndays >= days_until_year_rollover) {
      ...
      while (ndays) {
        ...
        if (LEAPYR(new_date->tm_year+1900) && new_date->tm_mon == 1) {
